...
const EditPortfolio: React.FC<PortfolioEditModalProps> = ({ isOpen, onClose, onSave }) => {
      ...
      if (imagePreviews.length > 0) {
        const files = await Promise.all(
          imagePreviews.map(async (preview, index) => {
            ...
            return new File([blob], `portfolio-${Date.now()}-${index}.${fileExtension}`, { type: mimeType })
          }),
        )
  ...
  return (
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold text-yellow">ADD PORTFOLIO</h2>
            ...
            <div className="border-2 border-dotted rounded-xl p-6 border-[#FCFBF726] text-center">
              <label className="flex flex-col items-center justify-center cursor-pointer">
                ...
                <div className="relative p-2 bg-[#FCF8E3] rounded-lg h-12 w-12 mb-2">
                  <Image src="/upload.png" alt="Upload" fill className="p-2" style={{ objectFit: "contain" }} />
                </div>
                <p className="font-bold text-[#04DF76] mb-1">Upload an image or video</p>
                <p className="text-[#D8D6CF] text-sm">Or drag and drop here (Max 4mb)</p>
              </label>
            </div>
          )}
          <p className="text-xs text-[#D8D6CF] mt-2">Max 5 images ({5 - imagePreviews.length} left)</p>
        </div>
